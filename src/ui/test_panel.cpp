	...
	void TestPanel::ConstructPopulationPlotCMD(char* cmd, int selected_run_intex)
	{
		...
		else if (obj_num >=4)
        {
            ...
            for (int i = 0; i < obj_num - 1; i++)
                sprintf(columns, "%s using %d title 'f%d',''", columns, i+1, i+1);
            sprintf(columns, "%s using %d title 'f%d'", columns, obj_num, obj_num);
            ...
            sprintf(cmd,
                ...
                "%s\n"
                "plot  '%s' %s\n"
                , res.description.c_str(),xtics,ytics, data_file, columns);
        }
		...
		for (int i = 0; i < calculations; i++)
		{
			...
			if (metric_name == "IGD")
			{
				if (res.igd_history.find(generations[i]) != res.igd_history.end())
					metric_value = res.igd_history[generations[i]];
				else
				{
					metric_value = CalculateIGD(pop, pop.size(), res.para.objective_num, res.para.problem_name);
					res.igd_history[generations[i]] = metric_value;
				}
			}
			else if (metric_name == "HV")
			{
				if(res.hv_history.find(generations[i]) != res.hv_history.end())
                    metric_value = res.hv_history[generations[i]];
                else
                {
                    int pf_size = 0;
                    double **pf_data = LoadPFData(pf_size, res.para.objective_num, res.para.problem_name);
                    HVCalculator hv_calculator(res.para.objective_num, pop.size());
                    //metric_value = hv_calculator.Calculate(pop, pop.size(), res.para.objective_num, pf_data, pf_size);
                    res.hv_history[generations[i]] = metric_value;
                }

			}
			else if (metric_name == "GD")
            {
                metric_value = CalculateGD(pop, pop.size(), res.para.objective_num, res.para.problem_name);
                //res.gd_history[generations[i]] = metric_value;
            }
            else if (metric_name == "IGDPlus")
            {
                //if(res.igdplus_history.find(generations[i]) != res.gdplus_history.end())
                //  metric_value = res.igdplus_history[generations[i]];
                //else
                //{
                    metric_value = CalculateIGDPlus(pop, pop.size(), res.para.objective_num, res.para.problem_name);
                    res.igdplus_history[generations[i]] = metric_value;
                //}

            }
            else if (metric_name == "GDPlus")
            {
                if(res.gdplus_history.find(generations[i]) != res.gdplus_history.end())
                    metric_value = res.gdplus_history[generations[i]];
                else
                {
                    metric_value = CalculateGDPlus(pop, pop.size(), res.para.objective_num, res.para.problem_name);
                    res.gdplus_history[generations[i]] = metric_value;
                }
            }
            else if(metric_name == "Spacing")
            {
                if(res.spacing_history.find(generations[i]) != res.spacing_history.end())
                    metric_value = res.spacing_history[generations[i]];
                else
                {
                    metric_value = CalculateSpacing(pop, pop.size(), res.para.objective_num);
                    res.spacing_history[generations[i]] = metric_value;
                }
            }

			
			fprintf(data_file, "%d\t%f\n", generations[i], metric_value);
		}
		fflush(data_file);
		fclose(data_file);
