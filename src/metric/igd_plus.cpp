...
namespace emoc {
	...
	double CalculateIGDPlus(const std::vector<std::vector<double>>& pop, int pop_num, int obj_num, std::string& problem_name)
    {
        //load pf data
        int pf_size = 0;
        ...
        for (int i = 0; i < pf_size; i++)
        {
            ...
            for (int j = 0; j < pop_num; j++)
            {
                ...
                for (int k = 0; k < obj_num; k++)
                {
                    new_distance[k] = pop[j][k] > pf_data[i][k] ? pop[j][k] - pf_data[i][k] : 0;
                    temp_distance += new_distance[k] * new_distance[k];
                }
                temp_distance = sqrt(temp_distance);
                if (min_distance > temp_distance)
