...
namespace emoc {
	...
	void Algorithm::PlotPopulation(Individual** pop, int gen)
	{
		...
		char pf_file_name[256];
		std::string problem_type = g_GlobalSettings->problem_name_.c_str();
		problem_type.pop_back();
		// printf("%s\n", problem_type.c_str());
		// sprintf(data_file_name, "./output/test_module/run%d/pop_%d.txt", current_run, gen);
		sprintf(data_file_name, "./output/%s/%s/%s/%d/pop_%d.txt", 
		g_GlobalSettings->algorithm_name_.c_str(),
		g_GlobalSettings->problem_name_.c_str(),
		g_GlobalSettings->weight_StringType_.c_str(),
		current_run, gen);
		sprintf(script_file_name, "./plotfile/%d.gnu", gen);
		switch (g_GlobalSettings->obj_num_)
		{
		case 2:
			sprintf(pf_file_name, "./pf_data/%s/%s.%dD.pf", 
			problem_type.c_str(),
			g_GlobalSettings->problem_name_.c_str(),
			...
			break;
		case 3:
			sprintf(pf_file_name, "./pf_data/data/%s", 
			g_GlobalSettings->problem_name_.c_str()
			);				
			break;
		default:
		...
		else if (g_GlobalSettings->problem_->encoding_ == Problem::REAL)
		{
			...
			{
				PlotManager::Instance()->Scatter2D(plot_cmd, gen, data_file_name, pf_file_name, algorithm_name, problem_name);
			}
			else if (g_GlobalSettings->obj_num_ == 3)
			{
				PlotManager::Instance()->Scatter3D(plot_cmd, gen, data_file_name, pf_file_name, algorithm_name, problem_name);
			}
