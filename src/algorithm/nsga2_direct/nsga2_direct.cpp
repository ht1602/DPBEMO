...
namespace emoc {
	...
	DPBEMO_NSGA2::DPBEMO_NSGA2(int thread_id) :
    Algorithm(thread_id),
    pf_size(0),
	usingPref(false),
	count(0),
	...
	void DPBEMO_NSGA2::Solve()
	{
		...
		if(g_GlobalSettings->problem_name_ == "RNA")
       	{
            CalculateMinDis(weight, g_GlobalSettings->parent_population_.data(), g_GlobalSettings->population_num_, g_GlobalSettings->obj_num_, g_GlobalSettings->dec_num_, g_GlobalSettings->problem_name_, g_GlobalSettings->algorithm_name_, g_GlobalSettings->RNA_ID_);
            CalculateExpectDis(weight, g_GlobalSettings->parent_population_.data(), g_GlobalSettings->population_num_, g_GlobalSettings->obj_num_, g_GlobalSettings->dec_num_, g_GlobalSettings->problem_name_, g_GlobalSettings->algorithm_name_, g_GlobalSettings->RNA_ID_);
       	}
       else
       	{
	...
	void DPBEMO_NSGA2::Initialization()
	{
	...
	void DPBEMO_NSGA2::Crossover(Individual **parent_pop, Individual **offspring_pop)
	{
	...
	void DPBEMO_NSGA2::SetDistanceInfo(std::vector<DistanceInfo> &distanceinfo_vec, int target_index, double distance)
	{
	...
	int DPBEMO_NSGA2::CrowdingDistance(Individual **mixed_pop,  int pop_num, int *pop_sort, int rank_index)
	{
		int num_in_rank = 0;
	...
	bool DPBEMO_NSGA2::EnvironmentalSelection(Individual **parent_pop, Individual **mixed_pop)
	{
    ...
    void  DPBEMO_NSGA2::UtilityFunction_Gaussian(Individual **pop)
    {
    ...
    void DPBEMO_NSGA2::StorePreference(Individual **pop)
    {
	...
	std::tuple<int, int, vec> DPBEMO_NSGA2::Consult_DM()
    {
        // mat PVals;
        int best;
	...
	int DPBEMO_NSGA2::FitnessViaPreference(Individual **mixed_pop, int pop_num, int *pop_sort, int rank_index, int nneed)
	{
		int num_in_rank = 0;
		std::vector<int> sort_arr(pop_num, 0);
		std::vector<DistanceInfo> distanceinfo_vec(pop_num, {-1, 0.0});
	...
	double DPBEMO_NSGA2::UpdateUtilityPrediction(double obj, int index)
	{
	...
	float DPBEMO_NSGA2::CalculateKLDivergence(Individual **pop)
	{
