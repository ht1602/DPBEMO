...
namespace emoc {
	...
	CNSGA2::CNSGA2(int thread_id) :Algorithm(thread_id)
	{
	...
	void CNSGA2::Crossover(Individual **parent_pop, Individual **offspring_pop)
	{
	...
	void CNSGA2::SetDistanceInfo(std::vector<DistanceInfo> &distanceinfo_vec, int target_index, double distance)
	{
	...
	int CNSGA2::CrowdingDistance(Individual **mixed_pop,  int pop_num, int *pop_sort, int rank_index)
	{
		int num_in_rank = 0;
	...
	void CNSGA2::EnvironmentalSelection(Individual **parent_pop, Individual **mixed_pop)
	{
		int current_popnum = 0, rank_index = 0;
		int mixed_popnum = g_GlobalSettings->population_num_ + 2 * (g_GlobalSettings->population_num_ / 2);

		NonDominatedSort(mixed_pop, mixed_popnum, g_GlobalSettings->obj_num_, true);
