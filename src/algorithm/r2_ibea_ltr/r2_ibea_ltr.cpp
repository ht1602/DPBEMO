...
namespace emoc {
	...
	void R2_IBEA_LTR::Solve()
	{
		...
		while (!IsTermination())
		{
            if( (g_GlobalSettings->iteration_num_ >= first_tau) && g_GlobalSettings->tau_ && (g_GlobalSettings->iteration_num_ % g_GlobalSettings->tau_ == 0))
            {
    ...
    void R2_IBEA_LTR::UpdateTrainingSet()
    {
        ...
        int index_1, index_2;
        int number_of_inquires = g_GlobalSettings->max_evaluation_/ (g_GlobalSettings->population_num_ * g_GlobalSettings->tau_);
