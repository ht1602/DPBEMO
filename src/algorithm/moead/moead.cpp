...
namespace emoc {
	...
	void MOEAD::Solve()
	{
		...
		{	
			for (int i = 0; i < weight_num_; ++i)
			{
				...
				Mutation(offspring, g_GlobalSettings->population_num_);
	...
	void MOEAD::Crossover(Individual **parent_pop, int current_index, Individual *offspring)
	{
		...
		{
			UniformCrossover(parent1, parent2, g_GlobalSettings->offspring_population_[1], offspring);
		}
		else if(g_GlobalSettings->problem_->encoding_ == Problem::PERMUTATION)
		{
			if(g_GlobalSettings->problem_name_ == "Antigen")
			{
				std::cout << "one point crossover" << "\n";
				OnePointCrossOver(parent1, parent2, g_GlobalSettings->offspring_population_[1], offspring);
			}
			else
			{
				std::cout << 'order crossover' << "\n";
				OrderCrossover(parent1, parent2, g_GlobalSettings->offspring_population_[1], offspring);
			}
		}
		
	}

	void MOEAD::Mutation(Individual* offspring, int pop_num)
	{
		if(g_GlobalSettings->problem_->encoding_ == Problem::REAL)
			PolynomialMutation(offspring, g_GlobalSettings->dec_lower_bound_, g_GlobalSettings->dec_upper_bound_, mutation_para_);
		else if(g_GlobalSettings->problem_->encoding_ == Problem::BINARY)
			BitFlipMutation(offspring, mutation_para_);
		else if(g_GlobalSettings->problem_->encoding_ == Problem::PERMUTATION)
		{
			mutation_para_.pro = 1.0 / g_GlobalSettings->population_num_;
			if(g_GlobalSettings->problem_name_ == "Antigen")
			{
				printf("the upper bound is %d\n", (int)g_GlobalSettings->dec_upper_bound_[0]);
				// SwapMutation(offspring, mutation_para_);
				UniformMutation(offspring, mutation_para_, (int)g_GlobalSettings->dec_upper_bound_[0]);
			}
			else
				SwapMutation(offspring, mutation_para_);
		}	
