#include "algorithm/nsga2_ppl/nsga2_ppl.h""
...
namespace emoc {
	...
	NSGA2PPL::NSGA2PPL(int thread_id):Algorithm(thread_id)
	{
	...
	void NSGA2PPL::Crossover(Individual **parent_pop, Individual **offspring_pop)
	{
	...
	void NSGA2PPL::SetDistanceInfo(std::vector<DistanceInfo> &distanceinfo_vec, int target_index, double distance)
	{
	...
	int NSGA2PPL::CrowdingDistance(Individual **mixed_pop,  int pop_num, int *pop_sort, int rank_index)
	{
		int num_in_rank = 0;
	...
	void NSGA2PPL::EnvironmentalSelection(Individual **parent_pop, Individual **mixed_pop)
	{
