...
namespace emoc {
	...
	int RVEA::ReferenceVectorGuidedSelection(Individual** parent_pop, Individual** mix_pop, int mix_pop_num)
	{
		...
		for (int i = 0; i < mix_pop_num; i++)
		{
			std::sort(angle_info[i].begin(), angle_info[i].end(), [](const DistanceInfo& left, const DistanceInfo& right) {
		...
		for (int i = 0; i < weight_num_; i++)
		{
			if (index[i] != 0)
			{
				std::sort(APD_info[i].begin(), APD_info[i].begin() + index[i], [](const DistanceInfo &left,const DistanceInfo &right) {
					return left.distance < right.distance;
					});
