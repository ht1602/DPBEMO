...
namespace emoc {
	...
	void MOEADSTM::CalculatePref(Individual** mixed_pop)
	{
		...
		for (int i = 0; i < weight_num_; i++)
		{
			std::sort(subproblem_matrix_[i], subproblem_matrix_[i] + size, [](const FitnessInfo& left, const FitnessInfo& right) {
		...
		for (int i = 0; i < size; i++)
		{
			std::sort(solution_matrix_[i], solution_matrix_[i] + weight_num_, [](const FitnessInfo& left, const FitnessInfo& right) {
				return left.fit < right.fit;
				});
		}
	}
