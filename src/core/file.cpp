...
namespace emoc {
	...
	static void PrintObjective(std::string &problem_name, const char *filename, int obj_num, Individual **pop_table, int pop_num)
	{
		FILE *fpt = fopen(filename, "w");
		...
		for (int i = 0; i < pop_num; ++i)
		{
			Individual *ind = pop_table[i];
			for (int j = 0; j < obj_num; ++j)
				if(problem_name == "Swimmer")
					fprintf(fpt, "%lf\t", -ind->obj_[j] + 300);
				else
					fprintf(fpt, "%lf\t", ind->obj_[j]);
			fprintf(fpt, "\n");
		}
	...
	void RecordPop(int run_index, int generation, Global* para, int real_popnum)
	{
		...
		else
		{
			...
			// );
			if(problem_name == "RNA")
					sprintf(output_dir, "./output/%s/%s/%d/%d_D%d/%s/%d/",
					algorithm_name.c_str(),
					problem_name.c_str(),
					para->RNA_ID_,
					para->obj_num_,
				...
				sprintf(output_dir, "./output/%s/%s/%d_D%d/%s/%d/",
					algorithm_name.c_str(),
					problem_name.c_str(),
					para->obj_num_,
					para->dec_num_,
		...
		sprintf(dec_file, "%sdec_%d.txt", output_dir, generation);
		if(para->obj_num_ == 1)
			PrintDecision(dec_file, para->dec_num_, para->parent_population_.data(), real_popnum);
		if (para->problem_->encoding_ == Problem::REAL)			
			// PrintObjective(para->problem_name_ ,output_file, para->obj_num_, para->parent_population_.data(), real_popnum);
	...
	void EMOCParamerterParse(int argc, char* argv[], EMOCParameters& para, bool& is_gui)
	{
		...
		// init parameter from connfig
		std::string Algorithm_name, Problem_name, Pop_size, Obj_num, Dec_num, Num_run, Evaluation, Pop_save_interval, Multithread, Num_thread, weight_StringType, tau, RNA_structure, RNA_ID;
		...
		try
		{
			...
			options
				...
				*/
				...
				("a,algorithm", "Algorithm name", cxxopts::value<std::string>()->default_value(config.Read("Algorithm_name",Algorithm_name)))
				("p,problem", "Problem name", cxxopts::value<std::string>()->default_value(config.Read("Problem_name",Problem_name)))
				("N,pop", "Population size", cxxopts::value<int>()->default_value(config.Read("Pop_size",Pop_size)))
				("M,obj", "Problem's objective dimension", cxxopts::value<int>()->default_value(config.Read("Obj_num",Obj_num)))
				("D,dec", "Problem's decision variable dimension", cxxopts::value<int>()->default_value(config.Read("Dec_num",Dec_num)))
				("e,evaluation", "Evaluation number for each run", cxxopts::value<int>()->default_value(config.Read("Evaluation",Evaluation)))
				("i,interval", "Population save interval", cxxopts::value<int>()->default_value(config.Read("Pop_save_interval",Pop_save_interval)))
				("r,run", "The number of runs", cxxopts::value<int>()->default_value(config.Read("Num_run",Num_run)))
				("multithread", "Whether open multi-thread mode", cxxopts::value<bool>()->default_value(config.Read("Multithread",Multithread)))	
				("t,thread", "Thread number in multi-thread mode", cxxopts::value<int>()->default_value(config.Read("Num_thread",Num_thread)))
				("A,algorithmparam","Other parameters of the algorithm, divided by ','",cxxopts::value<double>()->default_value("0.0"))
				("P,problemparam","Other parameters of  the problem, divided by ','",cxxopts::value<double>()->default_value("0.0"))
				// ("W,weightStringType","Weight for DM", cxxopts::value<std::string>()->default_value("0,0"))
				("W,weightStringType","Weight for DM", cxxopts::value<std::string>()->default_value(config.Read("Weight_DM", weight_StringType)))
				("q,elicitationInterval", "Frequency of Queries", cxxopts::value<int>()->default_value(config.Read("tau",tau)))
				// ("R,rnaStructure", "ID of RNA", cxxopts::value<std::string>()->default_value(config.Read("RNA_structure", RNA_structure)))
				("I,rnaID", "Frequency of Queries", cxxopts::value<int>()->default_value(config.Read("RNA_ID",RNA_ID)))
				;
			...
			para.weight_StringType=result["weightStringType"].as<std::string>();
			para.tau = result["elicitationInterval"].as<int>();
			if(para.problem_name == "RNA")
			{
				int line = result["rnaID"].as<int>();
				std::cout << line << std::endl;
				std::string target_strcture;
				target_strcture = ReadDataSet(line);
				std::cout << target_strcture << std::endl;
				para.RNA_ID = line;
				para.RNA_structure = target_strcture;
			}
			
			// para.RNA_structure = result["rnaStructure"].as<std::string>();
		}
		...
		if (!is_gui)
		{
			...
			std::cout << "------------EMOC Task------------\n"
				...
				<< "RNA_structure:   " << para.RNA_structure << "\n"
				...
				<< "DM weight:       " << para.weight_StringType << "\n"
				<< "tau:             " << para.tau << "\n"
				;
	...
	std::string ReadDataSet(int line)
	{
		std::ifstream dataset;
		dataset.open("./dataset/eterna100.txt");
		if(!dataset)
		{
		...
		std::cout << "finish loading dataset" << "\n";
		// std::cout << lines[line - 1] << "\n";
		return lines[line - 1];
	}
