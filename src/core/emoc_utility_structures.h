...
namespace emoc
{
	...
	public:
		...
		int thread_num;
		// double* algorithm_parameters;
		// double* problem_parameters;
		double algorithm_parameters;
		double problem_parameters;
		int algorithm_param_num;
		int problem_param_num;
		std::string weight_StringType;
		int tau;
		int RNA_ID;
		std::string RNA_structure;
	...
	public:
		EMOCParameters() :
			...
			thread_num(0),
			algorithm_parameters(0.0),
			problem_parameters(0.0),
			algorithm_param_num(0),
			problem_param_num(0),
			weight_StringType("0,0"),
			tau(0),
			RNA_ID(0),
			RNA_structure("(...)")
		{
	...
	struct EMOCMultiThreadResult
	{
		...
		EMOCMultiThreadResult(int run_num)
		{
			...
			igd_cf.metric_history.resize(run_num,0.0);
			igd_cf.is_record.resize(run_num,false);
		}
	};
	...
	struct EMOCSingleThreadResult
	{
		...
		std::unordered_map<int, double> igdcf_histrory;
