...
namespace emoc {
	...
	double CalculateInCos(double* a, double* b, int dimension)
	{
		return 1-CalculateDotProduct(a, b, dimension) / (CalculateNorm(a, dimension) * CalculateNorm(b, dimension));
	}
	...
	void CollectSingleThreadResult(int run_id, int thread_id, EMOCParameters para)
	{
		...
		if (para.objective_num > 1)				// multi-objective
		{
			...
			// result.igdcf_histrory[result.max_iteration]=igd_cf;
			...
			printf("run %d time: %fs   hv: %f \n", run_id, result.runtime, hv);
		...
		else if (para.objective_num == 1)		// single-objective
		{
			...
			result.igdcf_histrory[result.max_iteration] = -1;
		...
		py_res.igd_cf=result.lastigd_cf;
	...
	void CollectMultiThreadResult(int run_id, int parameter_id, int thread_id)
	{
		...
		if (g->obj_num_ > 1)				// multi-objective
		{
			...
			// double igd_cf=CalculateIGD_CF(g->parent_population_.data(),pop_num,obj_num,g->reference_point,0.005);
			double* reference_point=new double[dec_num];
			reference_point[0]=0.6;
			reference_point[1]=0.8;
			double igd_cf=CalculateIGD_CF(g->parent_population_.data(),pop_num,obj_num,reference_point,0.005);
			...
			if (g->current_evaluation_ >= g->max_evaluation_)
			{
				...
				res.igd_cf.metric_history[run_id]=igd_cf;
				...
				res.igd_cf.is_record[run_id] = true;
	...
	void StatisticTestAccordingMetric(EMOCMultiThreadResult& res, EMOCMultiThreadResult& compared_res, const std::string& metric, const std::string& format)
	{
		...
		else if (metric == "IGD-CF")
		{
			bool is_diff_ranksum = RankSumTest(res.igd_cf.metric_history, compared_res.igd_cf.metric_history);
			bool is_diff_signrank = SignRankTest(res.igd_cf.metric_history, compared_res.igd_cf.metric_history);
			if (res.igd_cf.metric_mean_ranksum[index] == -2) res.igd_cf.metric_mean_ranksum[index] = is_diff_ranksum ? (res.igd_cf.metric_mean < compared_res.igd_cf.metric_mean ? 1 : -1) : 0;
			if (res.igd_cf.metric_median_ranksum[index] == -2)res.igd_cf.metric_median_ranksum[index] = is_diff_ranksum ? (res.igd_cf.metric_median < compared_res.igd_cf.metric_median ? 1 : -1) : 0;
			if (res.igd_cf.metric_mean_signrank[index] == -2) res.igd_cf.metric_mean_signrank[index] = is_diff_signrank ? (res.igd_cf.metric_mean < compared_res.igd_cf.metric_mean ? 1 : -1) : 0;
			if (res.igd_cf.metric_median_signrank[index] == -2)res.igd_cf.metric_median_signrank[index] = is_diff_signrank ? (res.igd_cf.metric_median < compared_res.igd_cf.metric_median ? 1 : -1) : 0;
		}
		else
		{
	...
	void UpdateExpStatTest(int parameter_index)
	{
		...
		int igdcfmean_best_index = GetBestParameterIndex(range_start, range_end, "IGD-CF", "Mean");
		int igdcfmedian_best_index = GetBestParameterIndex(range_start, range_end, "IGD-CF", "Median");
		...
		for (int i = range_start; i < range_end; i++)
		{
			...
			if (i != range_end - 1)
			{
				...
				StatisticTestAccordingMetric(res, default_compared_res, "IGD-CF", "Default");
			}
			...
			if (i != igdcfmean_best_index)
			{
				EMOCMultiThreadResult& meanbest_compared_res = EMOCManager::Instance()->GetMultiThreadResult(igdcfmean_best_index);
				StatisticTestAccordingMetric(res, meanbest_compared_res, "IGD-CF", "Mean");
			}
			...
			if (i != igdcfmedian_best_index)
			{
				EMOCMultiThreadResult& medianbest_compared_res = EMOCManager::Instance()->GetMultiThreadResult(igdcfmedian_best_index);
				StatisticTestAccordingMetric(res, medianbest_compared_res, "IGD-CF", "Median");
			}
	...
	void UpdateExpResult(EMOCMultiThreadResult& res, int new_res_index, int parameter_index)
	{
		...
		UpdateExpMetricStat(res.igd_cf.metric_history, res.igd_cf.is_record, res.igd_cf.metric_mean, res.igd_cf.metric_std, res.igd_cf.metric_median, res.igd_cf.metric_iqr);
	}
	...
	void CalculateExpectDis(double *best, Individual** pop, int pop_num, int obj_num, int dec_num, std::string problem_name, std::string algorithm_name)
	{
		// double min_dis = 100;
        double dis, totaldis, expectdis;
		totaldis = 0.0;
        for(int i = 0; i < pop_num; ++i)
        {
            dis = CalEuclidianDistance(best,pop[i]->obj_.data(), obj_num);
            totaldis += dis;
        }
        expectdis = totaldis / pop_num;
		printf("Total dis %lf\n", totaldis);
        printf("Expected dis %lf\n", expectdis);
        ...
        sprintf(output_dir,"./ExpectDis/%s/",
	...
	void CalculateMinDis(double *best, Individual** pop, int pop_num, int obj_num, int dec_num, std::string problem_name, std::string algorithm_name)
    {
        double min_dis = 1e6;
        double dis;
        for(int i = 0; i < pop_num; ++i)
	...
	void CalculateExpectDis(double *best, Individual** pop, int pop_num, int obj_num, int dec_num, std::string problem_name, std::string algorithm_name, int RNA_id)
	{
		// double min_dis = 100;
        double dis, totaldis, expectdis;
		totaldis = 0.0;
        ...
        sprintf(output_file, "%s/%s_%d_M%d_D%d.txt", 
                output_dir, 
                problem_name.c_str(),
				RNA_id,
                obj_num,
	...
	void CalculateMinDis(double *best, Individual** pop, int pop_num, int obj_num, int dec_num, std::string problem_name, std::string algorithm_name, int RNA_id)
    {
        double min_dis = 1e6;
        double dis;
        for(int i = 0; i < pop_num; ++i)
