...
namespace emoc {
	...
	Global::Global(const char* algorithn_name, const char* problem_name, int population_num,
		...
		double algorithm_param_,int algorithm_param_num_,double problem_param_,int problem_param_num_, 
		const char *weight_StringType, int tau, int RNA_ID, const char* NRA_structure) :
		dec_num_(dec_num),
		obj_num_(obj_num),
		...
		// reference_point(nullptr)
	{
		...
		al_parameter_.num=algorithm_param_num_;
		al_parameter_.param=algorithm_param_;
		pr_parameter_.num=problem_param_num_;
		pr_parameter_.param=problem_param_;
	}
	...
	void Global::InitializeIndividual(Individual* ind)
	{
		...
		else if (problem_->encoding_ == Problem::PERMUTATION)
		{
			...
			if(problem_name_ == "Antigen")
			{
				// Generate a random integer within the range [0, 19]
				for (int i = 0; i < dec_num_; ++i)
				{
					ind->dec_[i] = rnd(0, 19);
					cout << ind->dec_[i] << " ";
	...
	void Global::InitializeProblem()
	{
		...
		if(problem_name_ == "RNA")
		{
			problem_->RNA_structure = new char[dec_num_ + 1];
        	strcpy(problem_->RNA_structure, RNA_structure_.c_str());
			std::cout << "RNA_structure" << problem_->RNA_structure << std::endl;
		}
		if (problem_ == nullptr)
		{
