...
namespace emoc {
	...
	void EMOCManager::EMOCSingleThreadRun()
	{
		...
		const char* weight_StringType = para_.weight_StringType.c_str();
		int tau = para_.tau;
		int RNA_ID = para_.RNA_ID;
		const char* RNA_structure = para_.RNA_structure.c_str();
		...
		for (int run = 0; run < para_.runs_num; ++run)
		{
			...
			g_GlobalSettingsArray[thread_id] = new emoc::Global(algorithm_name, problem_name, population_num,
				dec_num, obj_num, max_eval, thread_id, output_interval, run,para_.algorithm_parameters,para_.algorithm_param_num, para_.problem_parameters, para_.problem_param_num, weight_StringType, tau, RNA_ID, RNA_structure);
	...
	void EMOCManager::ExperimentWorker(std::vector<EMOCExperimentTask> tasks, int thread_id)
	{
		for (int i = 0; i < tasks.size(); i++)
		{
			...
			double algorithm_parameters = tasks[i].para.algorithm_parameters;
			int algorithm_param_num = tasks[i].para.algorithm_param_num;
			double problem_parameters = tasks[i].para.problem_parameters;
			int problem_param_num = tasks[i].para.problem_param_num;
			// para_.algorithm_parameters,para_.algorithm_param_num, para_.problem_parameters, para_.problem_param_num
			const char* weight_StringType = tasks[i].para.weight_StringType.c_str();
			int tau = tasks[i].para.tau;
			int RNA_ID = tasks[i].para.RNA_ID;
			const char* RNA_structure = tasks[i].para.RNA_structure.c_str();

			// algorithm main entity
			g_GlobalSettingsArray[thread_id] = new emoc::Global(algorithm_name, problem_name, population_num, dec_num, obj_num, max_eval, thread_id, output_interval, run_index, algorithm_parameters, algorithm_param_num,
			problem_parameters, problem_param_num, weight_StringType, tau, RNA_ID, RNA_structure);
			g_GlobalSettingsArray[thread_id]->Init();
			SetIsPlot(is_plot);
			g_GlobalSettingsArray[thread_id]->Start();
